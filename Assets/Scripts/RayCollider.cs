...
public class RayCollider : MonoBehaviour
{
    ...
    public bool playerColUp;
    public bool playerColDown;
    public bool playerColLeft;
    public bool playerColRight;
	...
	void checkCollision()
	{
        ...
        playerColDown = checkCollisionPlayer(raysDown);
        playerColUp = checkCollisionPlayer(raysUp);
        playerColLeft = checkCollisionPlayer(raysLeft);
        playerColRight = checkCollisionPlayer(raysRight);
	...
	bool checkCollision(List<Ray2D> rayList)
	{
		for (int i = 0; i < rayList.Count; i++)
		{
			...
			if(TileHit != null && TileHit.collider != null && !TileHit.collider.isTrigger && TileHit.collider.gameObject.tag != "Player")
			{
				return true;
			}
		}
    ...
    bool checkCollisionPlayer(List<Ray2D> rayList)
    {
        for (int i = 0; i < rayList.Count; i++)
        {
            //check all rays
